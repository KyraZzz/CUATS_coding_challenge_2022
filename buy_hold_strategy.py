...
class AlgoTradingCourse5(QCAlgorithm):
    """ Buy & hold ETF QQQ
        Place a limit order (an order for a specific price) to buy the security,
        And sell out all quantities bought if the price is 5% lower than the average filled price
        at the entry point. Then wait for 1 month before entering market again.
    """
    def Initialize(self):
    ...
    def OnOrderEvent(self, orderEvent):
        if orderEvent.Status != OrderStatus.Filled:
            return
        # send stop loss order if entry limit order if filled
